...
export function stepRun(state,level,world,actions,dt,cb={}){
  if(state.mode!=='playing'||state.paused)return;state.elapsed+=dt;
  ...
  const o=level.objectives[state.objectiveIndex];if(o){state.objectiveTime-=dt;if(state.objectiveTime<=0){state.streak=0;state.combo=1;state.objectiveTime=objectiveDeadline(level,state.objectiveIndex,state.difficulty)*.72;cb.deadline?.(o);}if(controlled.name===effectiveObjectiveCat(state,level)&&Math.hypot(controlled.x-o.x,controlled.z-o.z)<.72&&controlled.y<.25)objectiveComplete(state,level,cb);}
