...
export class AudioBus{
  constructor(){this.ctx=null;this.buffers=new Map();this.active=new Set();this.lastCat=0;this.catCooldown=6500;this.lastHenley=0;this.henleyCooldown=9000;this.lastByGroup=new Map();this.errors=[];this.timers=new Set();this.playCounts=new Map();}
  ...
  status(){return{ready:this.buffers.size,total:UNIQUE_FILES.length,errors:[...this.errors],active:this.active.size,voiceReady:[...this.buffers.keys()].filter(name=>name.startsWith('/assets/voice/')).length,plays:Object.fromEntries(this.playCounts)};}
