...
export class AudioBus{
  constructor(){this.ctx=null;this.buffers=new Map();this.active=new Set();this.lastCat=0;this.catCooldown=6500;this.lastByGroup=new Map();this.errors=[];}
  ...
  async preload(){await this.unlock();const jobs=[];for(const name of UNIQUE_FILES)if(!this.buffers.has(name))jobs.push(fetch(`/assets/audio/${name}`).then(r=>{if(!r.ok)throw new Error(`audio ${r.status}: ${name}`);return r.arrayBuffer();}).then(b=>this.ctx.decodeAudioData(b)).then(buf=>this.buffers.set(name,buf)).catch(error=>{const message=String(error?.message||error);if(!this.errors.some(e=>e.includes(name)))this.errors.push(`${name}: ${message}`);}));await Promise.all(jobs);}
  status(){return{ready:this.buffers.size,total:UNIQUE_FILES.length,errors:[...this.errors],active:this.active.size};}
