...
export class InputController {
  constructor(canvas,{hitInactiveCat,onDeviceChange=()=>{}}={}){
    ...
    this.device='pointer'; this.keys=new Set(); this.drag=null; this.actions={jump:false,switchCat:false,pause:false};
    this._bind();
  }
  ...
  _bind(){
    ...
    addEventListener('keydown',e=>{this._device('keyboard');const k=e.key.toLowerCase();this.keys.add(k);if([' ','q','tab','p','escape'].includes(k)){e.preventDefault();if(e.repeat)return;if(k===' ')this.actions.jump=true;if(k==='q'||k==='tab')this.actions.switchCat=true;if(k==='p'||k==='escape')this.actions.pause=true;}});
  ...
  sample(cameraYaw=0){
    ...
    return {steerDirection:{x:sx,z:sz},steerMagnitude:mag,jump:this.actions.jump,switchCat:this.actions.switchCat,pause:this.actions.pause,device:this.device,drag:this.drag?{x0:this.drag.x0,y0:this.drag.y0,x:this.drag.x,y:this.drag.y}:null};
  }
  consumeTransient(){this.actions.jump=false;this.actions.switchCat=false;this.actions.pause=false;}
