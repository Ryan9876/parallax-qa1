...
export class SceneView{
  constructor(canvas){
    ...
    this.scene=new THREE.Scene();this.scene.background=new THREE.Color(0xbec8bb);this.scene.fog=new THREE.Fog(0xbec8bb,18,36);
  ...
  build(level,world){
    ...
    const wallColor=kitchen?0xe8dfcf:0xe2eadf,backWallMat=mat(wallColor,null,{rough:.92}),sideWallMat=new THREE.MeshStandardMaterial({color:wallColor,roughness:.92,transparent:true,opacity:.14,depthWrite:false,side:THREE.DoubleSide});
    const backWall=box(13,.18,2.9,backWallMat,0,1.45,6.35),leftWall=box(.18,2.9,12.5,sideWallMat,-6.4,1.45,.2),rightWall=box(.18,2.9,12.5,sideWallMat,6.4,1.45,.2);leftWall.castShadow=rightWall.castShadow=false;leftWall.receiveShadow=rightWall.receiveShadow=false;leftWall.renderOrder=2;rightWall.renderOrder=2;this.worldGroup.add(backWall,leftWall,rightWall);
