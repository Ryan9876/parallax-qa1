...
export function createStorybookHenley(){
  ...
  // Storybook proportions: slightly larger head and hands, compact limbs, soft silhouettes.
  ...
  // Simple floral pattern gives the in-game model a clear link to the reference photo without chasing photorealism.
  ...
  // Skin face overlay keeps the front open while the cap reads as long hair from side/back views.
  const face=sphere(.225,skin);face.position.set(0,1.685,.045);face.scale.set(.92,.93,.88);body.add(face);
  for(const side of [-1,1]){
    const strand=capsule(.075,.58,hair);strand.position.set(side*.205,1.42,-.015);strand.rotation.z=side*.06;body.add(strand);
  }
  ...
  for(const side of [-1,1]){
    ...
    const leg=makePivot(body,side*.17,.54,0);const shin=capsule(.075,.43,skin);shin.position.y=-.26;leg.add(shin);const sockMesh=capsule(.077,.12,sock);sockMesh.position.set(0,-.52,.015);leg.add(sockMesh);const foot=shadow(new THREE.Mesh(new THREE.RoundedBoxGeometry(.16,.11,.25,3,.035),shoe));foot.position.set(0,-.60,.07);leg.add(foot);legs.push(leg);
  }
  ...
  function update(dt,{speed=0}={}){
    ...
    if(action==='call'){
      arms[1].rotation.x=damp(arms[1].rotation.x,-1.65,16,dt);arms[1].rotation.z=damp(arms[1].rotation.z,-.38,14,dt);body.rotation.y=Math.sin(phase*1.8)*.05;
    }else if(action==='celebrate'){
      arms[0].rotation.z=damp(arms[0].rotation.z,-1.65,14,dt);arms[1].rotation.z=damp(arms[1].rotation.z,1.65,14,dt);body.rotation.y=Math.sin(phase*2.1)*.09;
    }else{
      arms[0].rotation.z=damp(arms[0].rotation.z,0,12,dt);arms[1].rotation.z=damp(arms[1].rotation.z,0,12,dt);body.rotation.y=damp(body.rotation.y,0,10,dt);
    }
    body.position.y=damp(body.position.y,Math.abs(c)*.025*energy,14,dt);
    body.rotation.z=damp(body.rotation.z,-s*.018*energy,10,dt);
    head.rotation.y=damp(head.rotation.y,s*.022*energy,8,dt);
    toy.visible=holdingToy;
  }
