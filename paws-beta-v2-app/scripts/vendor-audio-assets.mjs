...
async function fetchSample(sample){
  ...
  return {...sample,provider:'BigSoundBank',creator:'Joseph SARDIN',license:'CC0 1.0',download:url,bytes:buf.length,sha256:crypto.createHash('sha256').update(buf).digest('hex'),local:`/assets/audio/${sample.file}`};
}
...
console.log(`Vendored ${records.length} CC0 sampled audio files (${(total/1024).toFixed(1)} KiB).`);
