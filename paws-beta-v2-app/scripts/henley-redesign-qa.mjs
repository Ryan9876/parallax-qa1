...
try{
  ...
  check('keyboard/direct movement advances Henley',Math.hypot(moved.henley.x-initial.state.henley.x,moved.henley.z-initial.state.henley.z)>.20,JSON.stringify({before:initial.state.henley,after:moved.henley}));
  ...
  await page.evaluate(()=>window.__PAWS_GAME__.completeRoundup());await page.waitForTimeout(120);try{await page.waitForFunction(()=>window.__PAWS_QA__?.mode==='success',null,{timeout:3000});}catch{}const done=await page.evaluate(()=>window.__PAWS_GAME__.snapshot());
  ...
  await mp.screenshot({path:path.join(outDir,'mobile-start.png'),fullPage:true});
